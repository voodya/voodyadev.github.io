...
var ASM_CONSTS = {
  3922704: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 3922765: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 3922829: function() {return Module.webglContextAttributes.powerPreference;},  
 3922887: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 3922942: function($0) {performance.now = function() { return $0; };},  
 3922990: function($0) {performance.now = function() { return $0; };},  
 3923038: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _CopyToClipboard(text) {
        ...
        navigator.clipboard.writeText(str).then(function() {
          ...
          window.prompt("Copy this link manually:", str);
        });
    }
  ...
  function _LogFirebaseEvent(eventName, params) {
            const eventNameStr = UTF8ToString(eventName);
            const paramsStr = UTF8ToString(params);
            window.logFirebaseEvent(eventNameStr, paramsStr);
    }

  function _OpenLink(linkPtr) {
        var link = UTF8ToString(linkPtr);
        window.open(link)
    }

  function _PurchaseProduct(url) {
      if (window.Telegram) {
        var invoiceLink = UTF8ToString(url);
        window.Telegram.WebApp.openInvoice(invoiceLink, function(status) {
          if (window.unityInstance){
              window.unityInstance.SendMessage("TelegramController", "SetPurchaseStatus", JSON.stringify({status: status}));
          }
        });
      }
    }

  function _RequestInitData() {
      if (window.Telegram) {
        if (window.unityInstance) {
          let initData = new URLSearchParams(window.Telegram.WebApp.initData).toString();
          console.log(initData);
          window.unityInstance.SendMessage("TelegramController", "SetWebAppUser", initData);
        }
      }
      else {
        window.unityInstance.SendMessage("TelegramController", "SetDefaultUser");
      }
    }

  function _SetUserId(userId) {
          const userIdStr = UTF8ToString(userId);
          window.setFirebaseUserId(userIdStr);
    }

  function _SetUserProperties(params) {
          const paramsStr = UTF8ToString(params);
          window.setUserProperties(paramsStr);
    }

  function _ShareReferralLink(referralLinkPtr, messagePtr) {
      var referralLink = UTF8ToString(referralLinkPtr);
      var message = UTF8ToString(messagePtr);
      var shareUrl = `https://t.me/share/url?url=${encodeURIComponent(referralLink)}&text=${encodeURIComponent(message)}`;
  ...
  var listener = {onRemoteConfigsUpdated:function()
          {
              SendMessage("GameAnalytics", "OnRemoteConfigsUpdated");
          }};
  function _addAdEvent(adAction, adType, adSdkName, adPlacement, fields, mergeFields) {
  ...
  function _addAdEventWithDuration(adAction, adType, adSdkName, adPlacement, duration, fields, mergeFields) {
          var fieldsString = UTF8ToString(fields);
          fieldsString = fieldsString ? fieldsString : "{}";
  ...
  function _addAdEventWithReason(adAction, adType, adSdkName, adPlacement, noAdReason, fields, mergeFields) {
          var fieldsString = UTF8ToString(fields);
          fieldsString = fieldsString ? fieldsString : "{}";
...
var asmLibraryArg = {
  ...
  "RequestInitData": _RequestInitData,
  "SetUserId": _SetUserId,
  "SetUserProperties": _SetUserProperties,
  "ShareReferralLink": _ShareReferralLink,
  ...
  "abort": _abort,
  "addAdEvent": _addAdEvent,
  "addAdEventWithDuration": _addAdEventWithDuration,
  "addAdEventWithReason": _addAdEventWithReason,
  "addBusinessEvent": _addBusinessEvent,
  "addDesignEvent": _addDesignEvent,
  "addDesignEventWithValue": _addDesignEventWithValue,
  "addErrorEvent": _addErrorEvent,
  "addProgressionEvent": _addProgressionEvent,
  "addProgressionEventWithScore": _addProgressionEventWithScore,
  "addResourceEvent": _addResourceEvent,
  "configureAvailableCustomDimensions01": _configureAvailableCustomDimensions01,
  "configureAvailableCustomDimensions02": _configureAvailableCustomDimensions02,
  "configureAvailableCustomDimensions03": _configureAvailableCustomDimensions03,
  ...
  "configureBuild": _configureBuild,
  "configureGameEngineVersion": _configureGameEngineVersion,
  "configureSdkGameEngineVersion": _configureSdkGameEngineVersion,
  "configureUserId": _configureUserId,
  ...
  "fd_seek": _fd_seek,
  "fd_write": _fd_write,
  "getABTestingId": _getABTestingId,
  "getABTestingVariantId": _getABTestingVariantId,
  "getRemoteConfigsContentAsString": _getRemoteConfigsContentAsString,
  "getRemoteConfigsValueAsString": _getRemoteConfigsValueAsString,
  ...
  "isRemoteConfigsReady": _isRemoteConfigsReady,
  "llvm_eh_typeid_for": _llvm_eh_typeid_for,
  "setCustomDimension01": _setCustomDimension01,
  "setCustomDimension02": _setCustomDimension02,
  "setCustomDimension03": _setCustomDimension03,
  "setEnabledInfoLog": _setEnabledInfoLog,
  "setEnabledVerboseLog": _setEnabledVerboseLog,
  "setEventSubmission": _setEventSubmission,
  "setGlobalCustomEventFields": _setGlobalCustomEventFields,
  "setManualSessionHandling": _setManualSessionHandling,
...
unexportedRuntimeFunction('listener', false);
unexportedRuntimeFunction('listener__user', false);
