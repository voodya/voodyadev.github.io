...
const cacheName = "VKContractors-Merge-2-0.8.6";
const contentToCache = [
    "Build/ecomerge.loader.js",
    "Build/ecomerge.framework.js.unityweb",
    "Build/ecomerge.data.unityweb",
    "Build/ecomerge.wasm.unityweb",
  ...
  e.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames
        .filter((name) => {
          return name.startsWith("VKContractors-Merge-2")
              && name !== cacheName;
        })
        .map((name) => {
